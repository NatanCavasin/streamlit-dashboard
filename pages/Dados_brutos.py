...
with st.sidebar.expander('Preço do produto'):
    min = dados['Preço'].min()
    max = dados['Preço'].max()
    preco = st.slider('Selecione o preço', min, max, (min, max))
with st.sidebar.expander('Frete da venda'):
    min = dados['Frete'].min()
    max = dados['Frete'].max()
    frete = st.slider('Frete', min, max, (min, max))
with st.sidebar.expander('Data da compra'):
    min = dados['Data da Compra'].min()
    max = dados['Data da Compra'].max()
    data_compra = st.date_input('Selecione a data', (min, max))
